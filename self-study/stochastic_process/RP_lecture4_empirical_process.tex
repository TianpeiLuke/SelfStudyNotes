...
\end{align}
where we have used the fact that $\sup_{f, g \in \cF}\norm{f - g}{\cP_n} \le 2b$. Then substituting the metric entropy bound for finite VC dimension class \eqref{ineqn: covering_number_bound_vc_dimension}, we have
\begin{align}
\E{\epsilon}{\sup_{f\in \cF}\abs{\frac{1}{n}\sum_{i=1}^n \epsilon_i f(x_i) }} &\le C_0\sqrt{\frac{d}{n}}\brac{1 + \int_{0}^{2b}\sqrt{\log(2b/t)}dt} = C_1 \sqrt{\frac{d}{n}} \label{ineqn: dudley_metric_entropy_Rademacher_process_vc_dim}
\end{align}
Finally, by \emph{symmetrization}, 
\begin{align*}
...
&\le C \sqrt{\frac{d}{n}}. \qed
\end{align*} 
\end{proof}

\item \begin{remark}
The inequality \eqref{ineqn: expected_emp_process_vc_dimension} provides a \emph{tighter} bound for \emph{the generalization error} in \emph{statistical learning}; i.e. with high probability
\begin{align*}
L(h) \le \widehat{L}_{n}(h) + C\sqrt{\frac{d}{n}}, 
\end{align*} as compared to the bound we obtain via \emph{Sauer-Shelah lemma}, which is $\cO(\sqrt{d\log(en/d) / n})$.
\end{remark}

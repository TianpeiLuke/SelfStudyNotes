...
\begin{itemize}
\item \begin{remark}
Consider \emph{the \textbf{suprema} of \textbf{empirical process}}:
\begin{align}
...
Consider a \textbf{countable} class of functions $\cF$ \textbf{uniformly bounded} by $b$. Then for all  $t > 0$, the suprema of empirical process $Z$ as defined in \ref{def: suprema_emp_process} satisfies the upper tail bound
\begin{align}
\cP\set{Z \ge \E{}{Z} + t} \le 2\exp\paren{-\frac{n t^2}{8e\E{}{\Sigma^2} + 4bt}} \label{ineqn: bernstein_inequality_functional}
\end{align} where $\Sigma^2 := \sup_{f \in \cF}\set{\frac{1}{n}\sum_{i=1}^{n}f^2(X_i)}$.
\end{theorem}
...
Let $X_1 \xdotx{,} X_n$ be independent identically distributed random vectors. Assume that $\E{}{f(X_i)} = 0$, and that $f(X_i) \le 1$ for all $f \in \cF$.
Let $\gamma^2= \sigma^2 + 2\E{}{Z}$ (where $\sigma^2 := \sup_{f\in \cF}\frac{1}{n}\sum_{i=1}^n\E{}{f^2(X_i)}$ is the wimpy variance). Let $\phi(u) = e^u - u - 1$ and $h(u) = (1 + u) \log(1 + u) - u$, for $u \ge -1$. Then for all $\lambda \ge 0$,
\begin{align*}
\log \E{}{e^{\lambda\paren{Z - \E{}{Z}}}} \le \gamma \phi(\lambda).
\end{align*}
Also, for all $t \ge 0$,
\begin{align}
...
\subsection{Bounding the Variance of Suprema of Empirical Process}

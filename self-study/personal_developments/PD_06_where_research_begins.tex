...
The \textbf{goal}: To \textbf{consolidate} all of the different types of writing you have produced during the Self-Centered Research process thus far into a single document.

Here is your checklist of items to compile:
\begin{itemize}
\item Digital notes. 
\item Handwritten notes. 
\item \textbf{Underlinings, highlights, and marginalia.} Return to any primary or secondary source that you have marked up in one form or another. 
...
\begin{enumerate}
\item \textbf{Tidy up (but only if it doesn’t bog you down). } 
\item \textbf{Note down ``self-evidence."} One important exception to the \textbf{Do Not Revise Yet rule}: during this transcription and consolidation process, stay centered and ``in tune" with yourself. 

Pay attention to any new thoughts or questions that surface while your consolidating your existing notes, and write those thoughts down directly into the Draft 0 digital file. 
\end{enumerate}
\end{exercise}

\vspace{20pt}
\item \begin{exercise}[Move from 0 to 1]
The \textbf{goal}: To create a ``Draft 1" -- a document with a (very) \textbf{preliminary sense of structure} -- by making an initial pass at sorting, grouping, and editing your consolidated writings from Draft 0.

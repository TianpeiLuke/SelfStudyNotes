...
\begin{itemize}
%\item \begin{remark}(\textbf{\emph{Two-Player Zero-Sum Game in Normal Form}})\\
%We study \emph{two-person games in \textbf{normal form}}. Such a game is defined by a matrix $M$. There are two players, \emph{\textbf{the row player}} and \emph{\textbf{the column player}}. To play the game, the row player chooses a row $i$ and, simultaneously, the column player chooses a column $j$. The selected entry $M(i, j)$ is \emph{\textbf{the loss} suffered by the \textbf{row player}}.
%
%The \emph{\textbf{row} player's goal} is to \emph{\textbf{minimize its loss}}, and we will generally focus mainly on this player's perspective of the game. Often, \emph{the goal of the \textbf{column} player} is to \emph{\textbf{maximize this loss}}, in which case the game is said to be \emph{\textbf{zero-sum}}, so named because the column player's loss can be viewed as the negative of the row player's, so that the losses of the two players always add up to exactly zero. 
%\end{remark}
...
%Consider \emph{\textbf{the sequential two-player zero-sum game}} as an \emph{online learning}.
%
%This game is played \emph{\textbf{repeatedly} in a sequence of rounds}. On round $t = 1 \xdotx{,} T$:
%\begin{enumerate}
%\item the \emph{\textbf{learner}} chooses \emph{mixed strategy} $P_t$;
%\item the \emph{\textbf{environment}} chooses \emph{mixed strategy} $Q_t$ (which may be chosen \emph{\textbf{with knowledge} of $P_t$});
%\item the \emph{learner} is permitted to \emph{\textbf{observe the loss}} 
%\begin{align*}
%M(i, Q_t) := \sum_{j=1}^{n}M_{i,j}Q_t(j)
%\end{align*} for each row $i$; this is the \emph{loss} it \emph{\textbf{would have} suffered} \emph{had it played using \textbf{pure strategy}} $i$ (i.e. \emph{the \textbf{potential outcomes}});
%\item the \emph{learner} suffers loss 
%\begin{align*}
% M(P_t, Q_t) = \sum_{i = 1}^{m}\sum_{j=1}^{n}P_{t}(i)M_{i,j}Q_{t}(j).
%\end{align*} 
%\end{enumerate}
...
When $N$ is large, solving the maximum entropy problem with $N$ linear constraints can be complex.  One way to solve \eqref{eqn: max_ent_boost} is by iteratively projecting the solution onto the subspace defined by one linear constraint. Since the optimization problem is convex, it is expected that this \emph{iterative projection} will converge to \emph{\textbf{the intersection of all linear subspaces}}. Figure \ref{fig: iterative_projection} illustrates the idea of iterative projection.

\emph{\textbf{AdaBoost}} can be seen to be an \underline{\emph{\textbf{iterative projection algorithm}}} that operates in \emph{\textbf{the space of probability distributions}} over \emph{\textbf{training examples}}.  For $t= 1 \xdotx{,} T$, the booster find  $\cD_{t+1}$ by minimizing the relative entropy with respect to $\cD_{t}$
\begin{align*}
&\min_{\cD} \kl{\cD}{\cD_t}\\

...
>
>
>The **generalized expectation-maximization algorithm** works as below:
...
>[!important] Definition
> the *(generalized) EM algorithm* is a **majorization-minimization (MM)** algorithm
>- Repeat the following steps until convergence:
>	- **M-Step**: *Estimate* $\hat{\theta}_{k+1}$ by **maximizing** the *proposed log-likelihood function* $$\hat{\theta}_{k+1} \in \arg\max_{\theta \in \Theta}\,Q(\theta; \hat{\theta}_{k})$$ where $Q(\theta; \hat{\theta}_{k}) := \mathbb{E}_{ p }\left[  \ell(\theta; x, z) \;|\; x, \hat{\theta}_{k} \right]$ is computed at **E-Step**.
>	- $k \leftarrow k+1$.
>
>Here $$Q(\theta; \hat{\theta}_{k}) := \mathbb{E}_{ p }\left[  \ell(\theta; x, z) \;|\; x, \hat{\theta}_{k} \right]$$ is the **surrogate function** in *MM algorithm* and
>$$
>Q(\theta; \hat{\theta}_{k}) = \log p_{\theta}(x) - \mathbb{KL}\left( p(\cdot | x, \hat{\theta}_{k}) \left\|\right. p(\cdot | x, \theta) \right).
>$$
>We have the following property of the surrogate function
>- If $\theta = \hat{\theta}_{k}$, then $$Q(\theta; \theta) = \log p_{\theta}(x).$$
>- For any $\theta\in \Theta$, $$Q(\theta; \hat{\theta}_{k}) \ge \log p_{\theta_{k}}(x)$$
>  

- [[Majorization-Minimization Algorithm]]


## EM Algorithm as E-Projection and M-Projection

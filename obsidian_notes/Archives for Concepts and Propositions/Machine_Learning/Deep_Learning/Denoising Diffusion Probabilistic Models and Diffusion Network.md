...
>- The sequence $$x_{0}\,\to\, z_{1} \,{\to}\ldots{\to}\,z_{T-1}\, \to\, z_{T}$$ is called the **forward pass** which is a **Markov chain** with **Gaussian transition function**.
...
>[!info]
>We now consider the *time-reversed Markov chain* $$z_{T} \to z_{T-1} \,{\to}\ldots{\to}\,z_{1} \to x$$
>Although the *forward transition kernel* is simple, the *reversed transition kernel*  $$\begin{align} Q(z_{t}, z_{t-1}) := q(z_{t-1} | z_{t}) &= \frac{q(z_{t}| z_{t-1})q(z_{t-1})}{q(z_{t})} \end{align}$$ is in general **intractable**, since the marginal distribution $$q(z_{t})= \int q(z_{t}|x)p(x)dx$$ depends on the *data distribution* $p(x)$, which is high dimensional and complex.
...
> - In other word, **given input** $x$, the **time-reversed Markov chain** can be generated by $$z_{t-1} = m_{t}(x, z_{t}) + \sqrt{ \beta_{t}\left(\frac{1 - \alpha_{t-1}}{1- \alpha_{t}}\right) }\;\epsilon_{t} $$
...
>- the second term is the *sum* of *forward* and *reverse transition kernels* $$\sum_{t=2}^{T}\log \left(\frac{p(z_{t-1}\,|\,z_{t}, w)}{q(z_{t}\,|\,z_{t-1}, x) }\right) := \sum_{t=2}^{T}\log \left(\frac{Q_{w}(z_{t}, z_{t-1})}{K_{x}(z_{t-1}, z_{t}) }\right)  $$
>  
...
>[!info]
>By Bayes theorem, we can *reverse the conditional distribution*
>$$
>\begin{align}
...
>\end{align}
>$$
>and the last term is independent from $w$, so we can drop this term.
>
...
>&\propto \mathbb{E}_{q }\left[\log p(x|Z_{1}, w) \right] - \sum_{t=2}^{T}\mathbb{E}_{Z_{t}\sim q(\cdot|x) }\left[\mathbb{KL}\left( q(Z_{t-1}| Z_{t}, x) \left\|\right. p(Z_{t-1}\,|\,Z_{t}, w) \right)\right] \\[5pt]
>&=\mathbb{E}_{Z_{1} \sim q(\cdot|x) }\left[\log p(x|Z_{1}, w) \right] - \frac{1}{2\beta_{t}}\sum_{t=2}^{T}\mathbb{E}_{Z_{t} \sim q(\cdot|x)  }\left[\lVert m_{t}(x, Z_{t}) - \mu(Z_{t}, w, t) \rVert_{2}^2\right] 
>\end{align*}
>$$  
>where
...
### Improvement on Decoder Network Formulation
...
>[!important] Definition
>Let $$g(z_{t}, w, t)$$ be a *neural network* that predicts the **difference** between the *corrupted data* from **forward pass** $z_{t}$ and *denoised data generated by* **diffusion model** $\mu(z_{t}, w,t)$ That is,
>$$\mu(z_{t}, w,t) = \frac{1}{\sqrt{ 1 - \beta_{t} }}\left[ z_{t} - \frac{\beta_{t}}{\sqrt{ 1 - \alpha_{t} }} g(z_{t}, w, t) \right] $$
>- Instead of predicting the *denoised data*, we **predict the total noise** added to $x$ in order to generate $z_{t}$
...
>&=-\sum_{t=1}^{T}\frac{\beta_{t}}{2(1- \alpha_{t})(1- \beta_{t})}\mathbb{E}_{Z_{t} \sim q(\cdot|x) }\left[\lVert g(Z_{t}, w, t) - \epsilon_{t} \rVert_{2}^2\right] 
>\end{align*}
>$$  
>
...
>$$
>\mathcal{L}(w) = \sum_{t=1}^{T}\mathbb{E}_{Z_{t} \sim q(\cdot|x) }\left[\lVert g(Z_{t}, w, t) - \epsilon_{t} \rVert_{2}^2\right] 
>$$
...
>	- *DDPM*:    a **fixed diffusion kernel**  + one **diffusion network** to approximate reversed transition kernel
>- **Training process**
>	- *VAE*: 
...
>	- *DDPM*: sample via a sequence of **time-reversed Markov chain**; $$z_{T}  \to \mu(z_{T}, w, T) \to z_{T-1} \to \mu(z_{T-1}, w, T-1) \,{\to}\ldots{\to}\,z_{1} \to \mu(z_{1}, w, 1) \to x$$
>- **Encoding step**
>	- *VAE*: 
...
>	- *DDPM*: A **multi-step diffusion process** with a *Gaussian  transition kernel* $$q(z_{k}|z_{k-1}, \beta_{t}) = \mathcal{N}(z_{k} | \sqrt{ 1 -\beta_{t} }z_{t-1}, \beta_{t}I)$$ 
>-  **Decoder**
>	- *VAE*: direct map from latent space to data space based on the  *decoder network* $$g(z; \theta) := p(x|z; \theta)$$
>	- *DDPM*: need a time-reversed Markov chain from latent space to data space based on the  *diffusion network* $$q(z_{t-1}|z_{t}) \approx p(z_{t-1}|z_{t}, w) := \mathcal{N}(z_{t-1}\;|\; \mu(z_{t}, w, t),\, \beta_{t}I)$$

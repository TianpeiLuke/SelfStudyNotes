...
>\end{align}
>$$
>and the last term is a **telescope sum**, $$\sum_{t=2}^{T}\log \left(\frac{q(z_{t-1}|x)}{q(z_{t}|x)}\right) = \sum_{t=2}^{T}\left(\log q(z_{t-1}|x) - \log q(z_{t}|x)\right) = \log q(z_{1}|x) - \log q(z_{T}|x)$$ which is independent from $w$, so we can drop this term.
>

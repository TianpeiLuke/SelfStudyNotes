...
## Forward Diffusion and Markov Chain
...
## Reverse Diffusion and Time-Reversed Markov Chain
...
>- the second term is the *sum* of *forward* and *reverse transition kernels* $$\sum_{t=2}^{T}\log \left(\frac{p(z_{t-1}\,|\,z_{t}, w)}{q(z_{t}\,|\,z_{t-1}, x) }\right)$$
>  
...
>\end{align}
>$$
>and the last term is a **telescope sum**, which is independent from $w$, so we can drop this term.
>
...
>&= \mathbb{E}_{q }\left[\log p(x|Z_{1}, w) \right] \\[5pt]

...
| **optimal substructure**        | the optimal solution is from a *combination* of the optimal solution of *each sub-problem* $$x^{*} = f(x^{(i)}, i\in I)$$ | the optimal solution is contained in each subproblem domain $$x^{*}\in S_{t}$$ and can be obtained by combining *subproblem solution* with *greedy choice*<br>$$x^{*} = g(x_{t}, a_{t})$$ | the optimal solution is obtained by the **Bellman equation**  *after* the optimal solution of *subproblem* in *next iteration* is obtained.  The optimal solution is the **fixed point** to the Bellman equation $$x_{t} \to x^{*}$$ We can show that $$V(x^{*}) \le \min_{a\in T(x^{*})}F(x^{*},a, V(z)),\; \forall z\in S_{t+1}$$ |

...
| optimal solution of subproblem  | optimal solutions of *subproblems* are *independent* $$x^{(i)} := \arg\min_{x\in S_{i}}V(x)$$                             | optimal solution depends *only* on the *greedy choice* $$x_{t} := \arg\min_{x\in S_{t}}V(x)$$                                                                                             | **Bellman optimality equation**: optimal solution depends on the *choice* which is function of optimal solution of *next iteration* $$V(x_{t}) = \min_{a\in S_{t}}F(x_{t}, a, V(x_{t+1}))$$                                                                                                                                      |
